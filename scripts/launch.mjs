...
async function main(){
 ...
 if(occupied){const html=await fetch(url).then(r=>r.text()).catch(()=>'');if(html.includes('<title>ECLucky13 v1.25</title>')){console.log('ECLucky13 is already running at '+url);console.log('Opening your browser — this window closes in 15 seconds.');open();await new Promise(r=>setTimeout(r,15000));return;}throw Error(`Port ${port} is in use. Close the other app or set PORT to another port.`)}
 ...
 while(!exited){try{const r=await fetch(url);if(r.ok){console.log('ECLucky13 v1.25 ready: HTTP '+r.status+' '+url);open();break}}catch{}await wait()}
}
