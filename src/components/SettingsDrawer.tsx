        ...
        <div className={'drawer-body' + (tab === 'prompt_studio' ? ' prompt-studio-body' : '')}>
          ...
          {tab === 'limits' && (
            <fieldset>
              ...
              <div className="field"><label>Cost-saver turn budget (0 = no budget)</label><input type="number" min="0" max="10000" disabled={!settings.provider.costSaver} value={settings.provider.saverTurnBudget} onChange={(e) => setP({ saverTurnBudget: Number(e.target.value) })} /><div className="hint">Warns at N turns, stops N+5 later. Applies only while cost saver is on. Default 60.</div></div>
