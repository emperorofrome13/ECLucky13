...
import { getChange, approveChange, workspaceLocked, type JournalEnv } from '@/server/workspace/change-journal';
...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  const active = runs.activeForWorkspace(wsPath);
  if (workspaceLocked(wsPath) || (active && (active.id !== change.runId || active.state !== 'waiting_for_approval'))) return Response.json({ ok: false, error: 'Wait for the workspace operation to finish.' }, { status: 409 });
  const env: JournalEnv = { workspacePath: wsPath, runId: change.runId, sessionId: change.sessionId, reviewMode: false };
  const r = approveChange(params.id, env);
